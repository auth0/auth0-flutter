...
extension AuthAPICustomTokenExchangeMethodHandlerTests {
    func testCallsCustomTokenExchange() {
        ...
        spy.onCustomTokenExchange = { subjectToken, subjectTokenType, audience, scope in
...
extension AuthAPICustomTokenExchangeMethodHandlerTests {
    func testWorksWithoutAudience() {
        ...
        spy.onCustomTokenExchange = { _, _, audience, _ in
    ...
    func testWorksWithEmptyScopes() {
        ...
        spy.onCustomTokenExchange = { _, _, _, scope in
            XCTAssertNil(scope)
            expectation.fulfill()
        }
        var args = arguments()
...
fileprivate extension SpyAuthentication {
    var onCustomTokenExchange: ((String, String, String?, String?) -> Request<Credentials, AuthenticationError>)?

    func customTokenExchange(subjectToken: String, 
                           subjectTokenType: String, 
                           audience: String?, 
                           scope: String?) -> Request<Credentials, AuthenticationError> {
        return onCustomTokenExchange?(subjectToken, subjectTokenType, audience, scope) ?? request()
    }
}
