...
flutter::EncodableValue JsonToEncodable(const web::json::value& v) {
  if (v.is_null()) return flutter::EncodableValue();

  if (v.is_boolean()) return flutter::EncodableValue(v.as_bool());
  if (v.is_number()) return flutter::EncodableValue(v.as_double());
  if (v.is_string())
