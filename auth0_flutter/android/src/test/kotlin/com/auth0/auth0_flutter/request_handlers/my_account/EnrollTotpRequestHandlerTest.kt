...
class EnrollTotpRequestHandlerTest {
    ...
    @Test
    fun `should call enrollTotp on the client`() {
        ...
        whenever(mockClient.enrollTotp()).thenReturn(mockRequest)

        handler.handle(mockClient, request, mockResult)

        verify(mockClient).enrollTotp()
    }
    ...
    fun `should call result success with challenge map on success`() {
        ...
        whenever(mockChallenge.id).thenReturn("totp|test123")
