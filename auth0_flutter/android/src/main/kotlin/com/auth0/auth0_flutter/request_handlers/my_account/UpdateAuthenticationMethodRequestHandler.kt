...
private const val MY_ACCOUNT_UPDATE_AUTH_METHOD_METHOD =
    "myAccount#updateAuthenticationMethod"

class UpdateAuthenticationMethodRequestHandler : MyAccountRequestHandler {
    override val method: String = MY_ACCOUNT_UPDATE_AUTH_METHOD_METHOD
    ...
    override fun handle(
        ...
        assertHasProperties(
            listOf("id"), request.data
        )

        val id = request.data["id"] as String
        val name = request.data["name"] as? String
        val preferredString =
            request.data["preferredAuthenticationMethod"] as? String
        val preferred = when (preferredString) {
            "voice" -> PhoneAuthenticationMethodType.VOICE
            "sms" -> PhoneAuthenticationMethodType.SMS
            else -> null
        }

        client.updateAuthenticationMethodById(id, name, preferred)
            .start(object :
